...
import type { CatalogoResponse, EstoqueResponse, Pedido } from '@/src/servicos/types/api.types';
...
export default function TelaHome() {
  ...
  // Função para obter mensagem de boas-vindas
  const getMensagemBoasVindas = () => {
    if (!estaAutenticado) {
      return 'Bem-vindo, Visitante!';
    }

    const nome = usuario?.nome || usuario?.email?.split('@')[0] || 'Usuário';
  ...
  const handleLogout = async () => {
    console.log('[Home] handleLogout chamado - estaAutenticado:', estaAutenticado);
    ...
    // Para web, usa window.confirm; para mobile, usa Alert
    if (Platform.OS === 'web') {
      const confirmou = window.confirm('Deseja realmente sair?');

      if (confirmou) {
        try {
          console.log('[Home] Usuário confirmou logout (web)');
          await logout();
          console.log('[Home] Logout concluído, redirecionando...');
          router.replace('/login');
        } catch (error) {
          console.error('[Home] Erro ao fazer logout:', error);
          window.alert('Erro: Não foi possível sair da conta');
        }
      } else {
        console.log('[Home] Logout cancelado (web)');
      }
    } else {
        ...
        [
          {
            text: 'Cancelar',
            style: 'cancel',
            onPress: () => console.log('[Home] Logout cancelado')
          },
          {
            text: 'Sair',
            style: 'destructive',
            onPress: async () => {
              try {
                console.log('[Home] Usuário confirmou logout');
                await logout();
                console.log('[Home] Logout concluído, redirecionando...');
                router.replace('/login');
              } catch (error) {
  ...
  const carregarPedidos = async () => {
    ...
    try {
      ...
      console.log('[Home] Carregando pedidos do cliente...');

      const response = await getMeusPedidos();
      setPedidos(response.pedidos || []);

      console.log('[Home] Pedidos carregados:', response.pedidos.length);
    } catch (error) {
      console.error('[Home] Erro ao carregar pedidos:', error);
      setPedidos([]);
    } finally {
  ...
  const estatisticasPedidos = useMemo(() => {
    const aguardandoConfirmacao = pedidos.filter(p => p.status === 'AGUARDANDO_CONFIRMACAO').length;
    const aguardandoPagamento = pedidos.filter(p => p.status === 'AGUARDANDO_PAGAMENTO').length;
    const aguardandoReceita = pedidos.filter(p => p.status === 'AGUARDANDO_RECEITA').length;
    const confirmado = pedidos.filter(p => p.status === 'CONFIRMADO').length;
    const emPreparacao = pedidos.filter(p => p.status === 'EM_PREPARACAO').length;
    const prontoParaEntrega = pedidos.filter(p => p.status === 'PRONTO_PARA_ENTREGA').length;
    const emTransporte = pedidos.filter(p => p.status === 'EM_TRANSPORTE').length;
    const entregue = pedidos.filter(p => p.status === 'ENTREGUE').length;
    const cancelado = pedidos.filter(p => p.status === 'CANCELADO').length;
    const recusado = pedidos.filter(p => p.status === 'RECUSADO').length;
  ...
  if (carregando) {
    return (
      <View style={styles.containerCarregando}>
        <ActivityIndicator testID="loading-indicator" size="large" color={temaMedico.cores.primaria} />
  ...
  return (
    <View testID="home-screen" style={styles.container}>
      ...
      >
        ...
        <View style={styles.cabecalho}>
          <View style={styles.cabecalhoConteudo}>
            ...
            <View style={styles.botoesNavegacao}>
              ...
              <TouchableOpacity
                style={[styles.botaoNavegacao, !estaAutenticado && styles.botaoDesabilitado]}
                onPress={() => estaAutenticado ? router.push('/pedidos') : null}
                disabled={!estaAutenticado}
              >
                <Ionicons
                  name="receipt-outline"
                  size={24}
                  color={estaAutenticado ? temaMedico.cores.primaria : temaMedico.cores.textoClaro}
                />
              </TouchableOpacity>
              <TouchableOpacity
                style={[styles.botaoNavegacao, !estaAutenticado && styles.botaoDesabilitado]}
                onPress={() => estaAutenticado ? router.push('/account') : null}
                disabled={!estaAutenticado}
              >
                <Ionicons
              ...
              >
                <Ionicons
                  name={estaAutenticado ? "log-out-outline" : "log-in-outline"}
                  size={24}
                  color={temaMedico.cores.primaria}
                />
                <Text style={styles.textoBotaoNavegacao}>
                  {estaAutenticado ? 'Sair' : 'Entrar'}
                </Text>
              </TouchableOpacity>
            </View>
          </View>
          <Text style={styles.boasVindas}>{getMensagemBoasVindas()}</Text>
        ...
        {estaAutenticado && !buscaAtiva && estatisticasPedidos.total > 0 && (
          <View style={styles.secao}>
            <View style={styles.secaoHeader}>
              <View style={styles.secaoHeaderConteudo}>
                <Ionicons name="receipt" size={22} color={temaMedico.cores.primaria} />
                <Text style={styles.tituloSecao}>Meus Pedidos</Text>
                <Text style={styles.contadorProdutos}>({estatisticasPedidos.total})</Text>
              </View>
              <TouchableOpacity
            ...
            >
              {estatisticasPedidos.aguardandoPagamento > 0 && (
                <TouchableOpacity
                  style={[styles.cardStatusPedido, { borderLeftColor: '#EF4444' }]}
                  onPress={() => router.push('/pedidos')}
                >
                  <View style={[styles.iconeStatusPedido, { backgroundColor: '#FEE2E2' }]}>
                    <Ionicons name="card" size={24} color="#EF4444" />
                  </View>
                  <Text style={styles.numeroStatusPedido}>{estatisticasPedidos.aguardandoPagamento}</Text>
                  <Text style={styles.tituloStatusPedido}>Aguardando Pagamento</Text>
                  <Text style={styles.subtituloStatusPedido}>Realizar pagamento</Text>
                </TouchableOpacity>
              )}
              {estatisticasPedidos.aguardandoReceita > 0 && (
                <TouchableOpacity
                  style={[styles.cardStatusPedido, { borderLeftColor: '#F59E0B' }]}
                  onPress={() => router.push('/pedidos')}
                >
                  <View style={[styles.iconeStatusPedido, { backgroundColor: '#FEF3C7' }]}>
                    <Ionicons name="document-text" size={24} color="#F59E0B" />
                  </View>
                  <Text style={styles.numeroStatusPedido}>{estatisticasPedidos.aguardandoReceita}</Text>
                  <Text style={styles.tituloStatusPedido}>Aguardando Receita</Text>
                  <Text style={styles.subtituloStatusPedido}>Enviar receita médica</Text>
                </TouchableOpacity>
              )}
              {estatisticasPedidos.confirmado > 0 && (
                <TouchableOpacity
                  style={[styles.cardStatusPedido, { borderLeftColor: '#06B6D4' }]}
                  onPress={() => router.push('/pedidos')}
                >
                  <View style={[styles.iconeStatusPedido, { backgroundColor: '#CFFAFE' }]}>
                    <Ionicons name="checkmark-done" size={24} color="#06B6D4" />
                  </View>
                  <Text style={styles.numeroStatusPedido}>{estatisticasPedidos.confirmado}</Text>
                  <Text style={styles.tituloStatusPedido}>Confirmado</Text>
                  <Text style={styles.subtituloStatusPedido}>Pedido confirmado</Text>
                </TouchableOpacity>
              )}
              {estatisticasPedidos.emPreparacao > 0 && (
                <TouchableOpacity
                  style={[styles.cardStatusPedido, { borderLeftColor: '#3B82F6' }]}
                  onPress={() => router.push('/pedidos')}
                >
                  <View style={[styles.iconeStatusPedido, { backgroundColor: '#DBEAFE' }]}>
                    <Ionicons name="cube" size={24} color="#3B82F6" />
                  </View>
                  <Text style={styles.numeroStatusPedido}>{estatisticasPedidos.emPreparacao}</Text>
                  <Text style={styles.tituloStatusPedido}>Em Preparação</Text>
                  <Text style={styles.subtituloStatusPedido}>Preparando pedido</Text>
                </TouchableOpacity>
              )}
              {estatisticasPedidos.prontoParaEntrega > 0 && (
                <TouchableOpacity
                  style={[styles.cardStatusPedido, { borderLeftColor: '#8B5CF6' }]}
                  onPress={() => router.push('/pedidos')}
                >
                  <View style={[styles.iconeStatusPedido, { backgroundColor: '#EDE9FE' }]}>
                    <Ionicons name="cube-outline" size={24} color="#8B5CF6" />
                  </View>
                  <Text style={styles.numeroStatusPedido}>{estatisticasPedidos.prontoParaEntrega}</Text>
                  <Text style={styles.tituloStatusPedido}>Pronto para Entrega</Text>
                  <Text style={styles.subtituloStatusPedido}>Aguardando coleta</Text>
                </TouchableOpacity>
              )}
              {estatisticasPedidos.emTransporte > 0 && (
                <TouchableOpacity
                  style={[styles.cardStatusPedido, { borderLeftColor: '#A855F7' }]}
                  onPress={() => router.push('/pedidos')}
                >
                  <View style={[styles.iconeStatusPedido, { backgroundColor: '#F3E8FF' }]}>
                    <Ionicons name="car" size={24} color="#A855F7" />
                  </View>
                  <Text style={styles.numeroStatusPedido}>{estatisticasPedidos.emTransporte}</Text>
                  <Text style={styles.tituloStatusPedido}>Em Transporte</Text>
                  <Text style={styles.subtituloStatusPedido}>A caminho</Text>
                </TouchableOpacity>
              )}
              {estatisticasPedidos.entregue > 0 && (
                <TouchableOpacity
                  style={[styles.cardStatusPedido, { borderLeftColor: '#10B981' }]}
                  onPress={() => router.push('/pedidos')}
                >
                  <View style={[styles.iconeStatusPedido, { backgroundColor: '#D1FAE5' }]}>
                    <Ionicons name="checkmark-circle" size={24} color="#10B981" />
                  </View>
                  <Text style={styles.numeroStatusPedido}>{estatisticasPedidos.entregue}</Text>
                  <Text style={styles.tituloStatusPedido}>Entregue</Text>
                  <Text style={styles.subtituloStatusPedido}>Pedido completo</Text>
                </TouchableOpacity>
              )}
            </ScrollView>
          </View>
